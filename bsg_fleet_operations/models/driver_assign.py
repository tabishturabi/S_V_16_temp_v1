...
class DriverAssign(models.Model):
    ...
    _description = 'All About Driver Assignment'
    ...
    model_id = fields.Many2one('fleet.vehicle.model', string='Vehicle Name')
    assignment_no = fields.Char(string='Assignment No.')
    ...
    assign_date = fields.Datetime(string='Assignment Date', default=fields.Datetime.today, track_visibility='onchange')
    assign_driver_id = fields.Many2one('hr.employee', string='Assignment Driver No/Name', track_visibility='onchange')
    ...
    unassign_id = fields.Many2one('driver.unassign', string='Unassign')
    document_ref = fields.Char(string='Reference/Description')
    fleet_vehicle_id = fields.Many2one('fleet.vehicle', string="Truck", track_visibility='onchange')
    ...
    comment = fields.Text(string='Vehicle Comment', store=True)

    # *----------------gernral information -------------*

    vehicle_type_id = fields.Many2one('bsg.vehicle.type.table', string='Vehicle Type Name')
    chassis_no = fields.Char(string='Chassis Number')
    truck_status_id = fields.Many2one('bsg.vehicle.status', string='Truck Status')
    manufactr_year = fields.Char(string='Manufacturing Year')
    plat_no = fields.Char(string='Plate No')
    comme = fields.Text(string='Comment')
    ...
    trailer_names = fields.Char(string='Trailer Name')
    comm = fields.Text(string='Comment')
    ...
    insurance = fields.Selection([
        ('yes', 'Yes'),
        ('no', 'No')], string='Insurance', required=True)
    insurance_reason = fields.Char(string='Insurance Reason')
    ...
    operation_card = fields.Selection([
        ('yes', 'Yes'),
        ('no', 'No')], string='Operation Card', required=True)
    operation_card_reason = fields.Char(string='Operation Card Reason')
    ...
    fleet_safety_certificate = fields.Selection([
        ('yes', 'Yes'),
        ('no', 'No')], string='Fleet Safety Certificate', required=True)
    fleet_safety_certificate_reason = fields.Char(string='Fleet Safety Certificate Reason')

    trailer_safety_certificate = fields.Selection([
        ('yes', 'Yes'),
        ('no', 'No')], string='Trailer Safety Certificate', required=True)
    trailer_safety_certificate_reason = fields.Char(string='Trailer Safety Certificate Reason')

    # @api.multi
    @api.onchange('pre_location_id', 'trailer_asset_status')
    def onchange_on_location(self):
        if self.pre_location_id:
            self.new_location_id = self.pre_location_id.id
        if self.trailer_asset_status:
            self.new_trailer_asset_status = self.trailer_asset_status.id

    @api.model
    def create(self, values):
        record = super(DriverAssign, self).create(values)
        if record.branch:
            record.assignment_no = 'DA' + record.branch + self.env['ir.sequence'].next_by_code('driver.assign')
        else:
            record.assignment_no = 'DA' + self.env['ir.sequence'].next_by_code('driver.assign')
        if values.get('fleet_vehicle_id'):
            fleet_id = self.env['fleet.vehicle'].search([('id', '=', int(values.get('fleet_vehicle_id')))], limit=1)
            fleet_id.write({
    ...
    # @api.multi
    def create_unassign(self):
        unassign_id = self.env['driver.unassign']
        unassign = unassign_id.create({
            'fleet_vehicle_id': self.fleet_vehicle_id.id,
            'assign_driver_id': self.assign_driver_id.id,
            'maintenence_work': self.maintenence_work,
            'document_ref': self.assignment_no,
            'serial_no': self.serial_no,
            'model_id': self.model_id.id,
            'unassign_date': self.assign_date,
    ...
    def action_confirm(self):
        ...
        if self.fleet_vehicle_id:
            ...
            if trailer_status_id:
                trailer_status_id.sudo(self.env.user.id).write({
                    'trailer_asset_status': self.new_trailer_asset_status.id,
                    'location_id': self.new_location_id.id
                })
            un_ass_employee_id = self.env['hr.employee'].search([('id', '=', self.unassign_driver_id.id)])
            un_ass_employee_id.sudo().write({
                'vehicle_sticker_no': ''
            })
            ...
            self.sudo(self.env.user.id).write(
                {'state': 'confirm', 'document_ref': unassign.unassignment_no, 'unassign_id': unassign.id})
            if self.assign_driver_id and self.assign_date and self.assignment_no:
                self.env['fleet.vehicle.assignation.log'].create(
                    {'vehicle_id': self.fleet_vehicle_id.id, 'bsg_driver_id': self.assign_driver_id.id,
                     'date_start': self.assign_date.date(), 'seq_name': self.assignment_no})
        return True
    ...
    # @api.multi
    def write(self, vals):
        res = super(DriverAssign, self).write(vals)
        if self.fleet_vehicle_id:
            self.fleet_vehicle_id.sudo().update({
                'kilometrage': self.kilometrage,
                'truck_license': self.truck_license,
                'insurance_card': self.insurance_card,
                'weight_card': self.weight_card,
    ...
    def check_trailer(self):
        if self.trailer_id:
            record_id = self.env['fleet.vehicle'].search(
                [('id', '!=', self.fleet_vehicle_id.id), ('trailer_id', '=', self.trailer_id.id)])
            if record_id:
                raise UserError(_("Trailer already linked with %s...!" % (record_id.taq_number)))

    # @api.multi
    @api.onchange('fleet_vehicle_id')
    def _get_driver_custody(self):
        if self.fleet_vehicle_id:
