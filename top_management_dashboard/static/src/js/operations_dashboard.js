odoo.define('operations_dashboard.Dashboard', function (require) {
"use strict";
...
var OperationsDashboard = AbstractAction.extend({
    template: 'OperationsDashboardMain',
//    cssLibs: [
...
//        'click .get_fuel_vouchers': 'get_fuel_vouchers',
        ...
        'click .get_transit_agreements': 'get_transit_agreements',
        'click .get_delivered_agreements': 'get_delivered_agreements',
...
//        "change select[name='checks']": '_onchange_checks',
    ...
    init: function(parent, context) {
        ...
        this.dashboards_templates = ['OperationsDetails'];
    },

    willStart: function() {
        var self = this;
        console.log("............Inside will start of operations dashboard..............")
    ...
    _onchange_select_timeline:function(e){
     ...
     on_reverse_breadcrumb: function() {
        ...
        this.fetch_data_by_timeline().then(function() {
            self.$('.operations_dashboard').empty();
            self.render_dashboards();
        });
    },
...
//        },options)
//    },
    get_transit_agreements:function(e){
        var self = this;
//        e.stopPropagation();
//        e.preventDefault();
        var options = {
        ...
        this.do_action({
            name: _t("Transit Agreements"),
            type: 'ir.actions.act_window',
            res_model: 'bsg_vehicle_cargo_sale_line',
            view_mode: 'tree,form',
            view_type: 'form',
            views: [[false, 'list'],[false, 'form']],
            domain: [['id','in', this.agreements.agreements_ready_to_ship_ids]],
            target: 'current'
        },options)
    },
     get_delivered_agreements:function(e){
        var self = this;
//        e.stopPropagation();
//        e.preventDefault();
        var options = {
        ...
        this.do_action({
            name: _t("Delivered Agreements"),
            type: 'ir.actions.act_window',
            res_model: 'bsg_vehicle_cargo_sale_line',
            view_mode: 'tree,form',
            view_type: 'form',
            views: [[false, 'list'],[false, 'form']],
            domain: [['id','in', this.agreements.agreements_delivered_ids]],
            target: 'current'
        },options)
    },
...
//    },
    ...
    render_dashboards: function() {
        var self = this;
        if (this.trips){
            var templates = ['OperationsDetails']
            _.each(templates, function(template) {
                self.$('.operations_dashboard').empty();
                self.$('.operations_dashboard').append(QWeb.render(template, {widget: self}));
            });
        }
        else{
                self.$('.o_hr_dashboard').empty();
                self.$('.o_hr_dashboard').append(QWeb.render('OperationsWarning', {widget: self}));
            }
    },
});


core.action_registry.add('top_management_dashboard.operations_dashboard', OperationsDashboard);
