...
class AutoVacuum(models.AbstractModel):
    ...
    def power_on(self, *args, **kwargs):
        ...
        rules = self.env['muk_autovacuum.rules'].sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).search([], order='sequence asc')
        for rule in rules:
            if rule.state in ['time', 'size', 'domain']:
                model = self.env[rule.model.model].sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id)
                ...
                if rule.only_attachments:
                    attachments = self.env['ir.attachment'].sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).search([
