...
class AutoVacuumRules(models.Model):
    ...
    def _default_sequence(self):
        record = self.sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).search([], order='sequence desc', limit=1)
        if record.exists():
    ...
    def _onchange_model(self):
        field_domain = [
            ...
            ('ttype', '=', 'datetime'),
            ('name', '=', 'create_date')]
        model = self.env['ir.model.fields'].sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id)
    ...
    def _check_code(self):
        for record in self.sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).filtered('code'):
