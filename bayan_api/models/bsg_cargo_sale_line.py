...
class BsgCargoSaleLine(models.Model):
    ...
    waybill_id = fields.Char(string='Bayan Waybill ID', related="bayan_way_bill_line_id.waybill_id")
    bayan_config_active = fields.Boolean(string="Is Bayan Config Active", compute="_compute_baya_config_id")
    bayan_way_bill_line_id = fields.Many2one('way.bill.line', track_visibility=True, string="Way Bill ID")
    way_bill_line_state = fields.Selection([('closed', 'Closed'), ('cancelled', 'Cancelled')],
                                           string="Bayan Status", track_visibility=True,
                                           related='bayan_way_bill_line_id.state')
    ...
    def action_Delivered(self):
        res = super(BsgCargoSaleLine, self).action_Delivered()
        if self.bayan_config_active:
            if self.bayan_way_bill_line_id and self.way_bill_line_state not in ['close', 'cancelled']:
                self.action_close_waybill()
        return res
