...
class AccountMove(models.Model):
    ...
    # override to pass state to reversal entry on voucher
    # @api.multi
    def write(self, vals):
        if self.reversed_entry_id:
            if self.line_ids:
                if self.reversed_entry_id.payment_id:
                    if len(self.reversed_entry_id.payment_id.bsg_vehicle_cargo_sale_line_ids) > 0:
                        self.reversed_entry_id.payment_id.bsg_vehicle_cargo_sale_line_ids.unlink()
                    self.reversed_entry_id.payment_id.write({'state': 'reversal_entry'})
