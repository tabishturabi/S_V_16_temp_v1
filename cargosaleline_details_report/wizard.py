...
class XlsxReportCargisaleline(models.TransientModel):
	...
	g_state = fields.Selection(
		[
			('all', 'All'),
			('posted', 'Posted'),
			('non_posted', 'Non Posted'),
			
		], default='all',string="General State")
	...
	report_type = fields.Selection(string="Report Type",default="01", selection=[
		('01','‫بوسيلة‬ الدفع‬'),
		('02','‫المرحلة‬ ‫وغير‬ ‫المرحلة‬'),
		('03','‫المسلمة‬ ‫وغير‬ ‫المسلمة‬'),
		('04','‫القيمة‬ ‫بإجمالي‬'),
	],required=True)
	...
	inv_state = fields.Selection([
		...
		('without_invoice', 'Without Invoice')], string='Invoice State', required=True,default="all")
