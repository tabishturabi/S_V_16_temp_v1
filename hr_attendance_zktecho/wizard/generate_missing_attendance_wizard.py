...
class generate_missing_attendance(models.TransientModel):
    ...
    def generate_attendance(self):
        try:
                    ...
                    if emp:
                        ...
                        for attendance_day in atten[emp]:
                            ...
                            if len(day_dict) == 1 and not is_shifted_employee:
                                date = datetime.strptime(str(day_dict[0].name), '%Y-%m-%d %H:%M:%S')
                                if day_dict[0].attendance_status == 'sign_in':
                                    ...
                                    if date.strftime('%A') == 'Friday':
                                        ...
                                        hour = self.get_day_worktime(day_dict[0].employee_id, 
                                                          date.strftime('%A'), day_dict[0].name, 'out')[0]
                                else:
                                    action = 'sign_in'
                                    if date.strftime('%A') == 'Friday':
                                        hour = self.off_get_day_worktime(day_dict[0].employee_id, 
                                        ...
                                        hour = self.get_day_worktime(day_dict[0].employee_id, 
                                                      date.strftime('%A'), day_dict[0].name, 'in')[0]
                                if hour:
                                    ...
                                    if f_action == 'sign_in':
                                        ...
                                        if fl_date.strftime('%A') == 'Friday':
                                            hour = self.off_get_day_worktime(last_date.employee_id, 
                                                              fl_date.strftime('%A'), last_date.name, 'out')[0]
                                        else:
                                        ...
                                        if fl_date.strftime('%A') == 'Friday':
                                            hour = self.off_get_day_worktime(last_date.employee_id, 
                                                              fl_date.strftime('%A'), last_date.name, 'in')[0]
                                        else:
                                            hour = self.get_day_worktime(last_date.employee_id, 
                                                              fl_date.strftime('%A'), last_date.name, 'in')[0]
                                    if hour:
            ...
            raise UserError("The following error occured while generating missing attendances.\n\n" + str(e))
    ...
    def get_day_worktime(self, employee, day_id ,date, action):
        ...
        for day in employee.resource_calendar_id.attendance_ids:
            ...
            local_date = my_local_timezone.localize(datetime.strptime(str(date),'%Y-%m-%d %H:%M:%S'))
        ...
        date = str(datetime.strptime(str(date),'%Y-%m-%d %H:%M:%S').date())
        if time_hour:
            #
    ...
    def shifted_employee(self, employee ,date):
        date_time = datetime.strptime(str(date)+' 00:00:00', '%Y-%m-%d %H:%M:%S')
        day_of_week = {'Monday':0 ,'Tuesday':1 ,'Wednesday':2 ,'Thursday':3 ,'Friday':4 ,'Saturday':5 ,'Sunday':6 }
    ...
    def is_same_shift(self, day_dict):
        f_attendance = datetime.strptime(str(day_dict[0].name), '%Y-%m-%d %H:%M:%S')
        l_attendance = datetime.strptime(str(day_dict[1].name), '%Y-%m-%d %H:%M:%S')
