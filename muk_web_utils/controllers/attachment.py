...
class AttachmentController(http.Controller):
    ...
    def add_attachment(self, ufile, temporary=False, **kw):
        ...
        if ufile.mimetype and ufile.mimetype != 'application/octet-stream': 
            attachment.sudo().with_context(force_company=request.env.user.company_id.id, company_id=request.env.user.company_id.id).write({
                'mimetype': ufile.mimetype,
            })
        base_url = request.env['ir.config_parameter'].sudo().with_context(force_company=request.env.user.company_id.id, company_id=request.env.user.company_id.id).get_param('web.base.url')
        result = attachment.read(['name', 'store_fname', 'mimetype', 'checksum', 'access_token'])[0]
