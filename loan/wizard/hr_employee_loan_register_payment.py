...
class HrLoanRegisterPaymentWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        res = super(HrLoanRegisterPaymentWizard, self).default_get(fields)
    ...
    def _default_amount(self):
        ...
        loan_ids = self.env['loan.application'].browse(active_ids)
        loan_states = set(loan_ids.mapped('state'))
        if not all(state == 'approved' for state in loan_states):
            raise ValidationError('You can only register payment for Approved Loan.')
        total_net = sum(loan_ids.mapped('approved_loan_amt'))
        return total_net
