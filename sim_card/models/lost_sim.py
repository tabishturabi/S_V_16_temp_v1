...
class LostRequest(models.Model):
    ...
    _description = "Lost Sim Card Upgrade"
    _rec_name = "name"
    ...
    state = fields.Selection(string="Sim Card Type", selection=[('draft', 'Draft'),
                                                                ('submitted', 'Submitted'),
                                                                ('done', 'Done'), ('cancel', 'Cancel')],
                             default='draft', track_visibility=True)
    my_employee_readonly = fields.Boolean(string='Employee Readonly')
    attachment_ids = fields.Many2many('ir.attachment')
    mble_no = fields.Many2one('sim.card.define', string='Mobile Number', track_visibility=True, required=True )
    ...
    transaction_type = fields.Selection(string="Transaction type", selection=[('lost sim', 'Lost SIM')],
                                        default='lost sim', required=True, track_visibility=True)
    ...
    def _compute_attachment_number(self):
        attachment_data = self.env['ir.attachment'].read_group(
            [('res_model', '=', 'lost.request'), ('res_id', 'in', self.ids)], ['res_id'], ['res_id'])
    ...
    def action_get_attachment_view(self):
        self.ensure_one()
        res = self.env['ir.actions.act_window']._for_xml_id('sim_card.action_attachment')
        return res
    ...
    def create(self, vals):
        res = super(LostRequest, self).create(vals)
        if self.env.user.user_branch_id.branch_no:
            res.name = 'RLSR' + self.env.user.user_branch_id.branch_no + self.env['ir.sequence'].next_by_code(
                'lost.request')
        else:
            res.name = 'RLSR' + self.env['ir.sequence'].next_by_code('lost.request')
        return res
    ...
    def action_submit(self):
        ...
        MailTemplate = self.env.ref('sim_card.mail_sim_lostt_request_tmpl', False)
        ...
        msg_id = self.env['mail.message'].search([('model', '=', 'lost.request'), ('res_id', '=', self.id)])
        msg_id.unlink()
        return True
    ...
    def action_send_mail(self):
        ...
        try:
            template_id = self.env.ref('sim_card.sim_card_lost_email_template', False)
        except ValueError:
            template_id = False
        try:
