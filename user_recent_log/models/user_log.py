...
class UserRecentLog(models.Model):
    ...
    def get_record(self, model, res_id):
        return self.env[model].sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).browse(res_id)
    ...
    def get_recent_log(self, model, res_id, changes=False):
        ...
        if not changes and model != "user.recent.log":
            self.sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).create({'model': model,
                                'res_id': res_id,
                                'user_id': user,
                                'last_visited_on': current_time})
        if changes:
            response_text = 'Please find following User Activities: \n'
            recent_record = self.sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).search([], order='id desc', limit=1)
            fields_data = record.sudo().with_context(force_company=self.env.user.company_id.id, company_id=self.env.user.company_id.id).read(changes.keys())[0]
